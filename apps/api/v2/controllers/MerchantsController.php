...
QUERY;
		if ($this->_premium_merchant) {
			...
			if ($search_query) {
				...
				$query                .= ' AND (a.company LIKE ? OR e.name LIKE ? OR f.name LIKE ?';
				foreach (range(1, 3) as $i) {
					$params[] = "%{$filtered_search_query}%";
				}
				if (count($filtered_keywords) > 1) {
					foreach ($filtered_keywords as $keyword) {
						$query .= ' OR a.company LIKE ? OR e.name LIKE ? OR f.name LIKE ?';
						foreach (range(1, 3) as $i) {
							$params[] = "%{$keyword}%";
						}
					}
