	...
	<div class="inner-wrapper">
		...
		<section role="main" class="content-body">
			<header class="page-header">
				<a href="/admin/users/{{ user.id }}/service_areas{% if page.current > 1%}/index/page:{{ page.current }}{% endif %}"><h2>Area Operasional</h2></a>
					...
					<div class="tab-content">
						<div id="areas" class="tab-pane active">
							...
							<p style="margin-left:5px" class="text-right">
								<a type="button" href="/admin/users/{{ user.id }}/service_areas/update{% if page.current > 1 %}/page:{{ page.current }}{% endif %}" class="btn btn-info"><i class="fa fa-pencil"></i> Update Minimum Order</a>
							</p>
							{% endif %}
							<table class="table table-striped">
								<thead>
	...
	for (let i = items.length; i--; ) {
		...
		item.onclick = () => {
			if (confirm('Anda yakin menghapus data ini ?')) {
				...
				form.method = 'POST',
				form.action = '/admin/users/' + item.dataset.userId + '/service_areas/' + item.dataset.id + '/delete{% if page.current > 1%}?page={{ page.current }}{% endif %}',
