	...
	<script>
		let attachEvent = () => {
			for (let notifications = document.querySelectorAll('.notification'), i = notifications.length; i--; ) {
				...
				notification.onclick = () => {
					fetch('/web_notifications/update/' + notification.dataset.id + '/read:1', { credentials: 'include', method: 'POST' }).then(() => {
		...
		attachEvent(),
		setInterval(() => {
			fetch('/home/inbox', { credentials: 'include' }).then(response => {
				...
				try {
					...
					location.href = '/sessions/create?next=' + location.pathname + location.search;
					return
				}
			})
		}, 6e4),
