	...
	<div class="inner-wrapper">
		...
		<section role="main" class="content-body">
			<header class="page-header">
				<a href="/coverage_areas{% if page.current > 1%}/index/page:{{ page.current }}{% endif %}"><h2>Area Operasional</h2></a>
			...
			<header class="panel-heading">
				<h2 class="panel-title">Area Operasional</h2>
			</header>
			<div class="panel-body">
				<!-- Content //-->
				{{ flashSession.output() }}
				<form method="POST" action="/coverage_areas/create">
				...
				<p style="margin-left:5px" class="text-right">
					<a type="button" href="/coverage_areas/update{% if page.current > 1 %}/page:{{ page.current }}{% endif %}" class="btn btn-info"><i class="fa fa-pencil"></i> Update Minimum Order</a>
				</p>
				{% endif %}
				<table class="table table-striped">
					<thead>
				...
				<div class="weepaging">
					<p>
						...
						{% for i in pages %}
							...
							<b>{{ i }}</b>
							{% else %}
							<a href="/coverage_areas{% if i > 1 %}/index/page:{{ i }}{% endif %}">{{ i }}</a>
							{% endif %}
						{% endfor %}
					</p>
				</div>
	...
	for (let i = items.length; i--; ) {
		...
		item.onclick = () => {
			if (confirm('Anda yakin menghapus data ini ?')) {
				...
				form.method = 'POST',
				form.action = '/coverage_areas/' + item.dataset.id + '/delete{% if page.current > 1%}?page={{ page.current }}{% endif %}',
