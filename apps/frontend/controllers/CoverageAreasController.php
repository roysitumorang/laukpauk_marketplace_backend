...
class CoverageAreasController extends ControllerBase {
	...
	function updateAction() {
		...
		if ($this->request->isPost()) {
			...
			return $this->response->redirect('/coverage_areas' . ($page > 1 ? '/index/page:' . $page : ''));
		}
		$this->_render($coverage_area);
	}
	...
	function deleteAction($id) {
		...
		if ($this->request->isPost()) {
			if (!($coverage_area = CoverageArea::findFirst(['user_id = ?0 AND village_id = ?1', 'bind' => [$this->currentUser->id, $id]]))) {
	...
	private function _render(CoverageArea $coverage_area = null) {
		...
		$result       = $this->db->query(<<<QUERY
			...
			WHERE
				b.id = {$this->currentUser->village->subdistrict->city->id} AND
				NOT EXISTS(SELECT 1 FROM coverage_area e WHERE e.village_id = d.id AND e.user_id = {$this->currentUser->id})
