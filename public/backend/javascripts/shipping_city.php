...
$sql = "SELECT a.vKecamatan AS vKecamatan, a.id AS idKecamatan, b.id AS idCity, b.vCity AS vCity, (SELECT id FROM cpstate WHERE id=b.idState) AS id, (SELECT vState FROM cpstate WHERE id=b.idState) AS vState FROM cpkecamatan a JOIN cpcity b ON b.id=a.idCity WHERE (a.vKecamatan LIKE '%".escape_str($term, $conn)."%') OR (b.vCity LIKE '%".escape_str($term, $conn)."%')";
$sql .= " ORDER BY b.id ASC";

$result = mysql_query($sql);
while($row = mysql_fetch_assoc($result)) {
  $a_json_row["id"] = $row['idKecamatan'];
  $a_json_row["value"] = $row['vCity']." - ".$row['vKecamatan']; //$row['idKecamatan'];
  $a_json_row["label"] = $row['vCity']." &raquo; ".$row['vKecamatan'];
  array_push($a_json, $a_json_row);
}

/*
$conn = new mysqli($App['db']['host'],$App['db']['user'],$App['db']['pass'],$App['db']['name']);
...
/*
$sql = "SELECT a.vKecamatan AS vKecamatan, a.id AS idKecamatan, b.id AS idCity, b.vCity AS vCity, (SELECT id FROM cpstate WHERE id=b.idState) AS id, (SELECT vState FROM cpstate WHERE id=b.idState) AS vState FROM cpkecamatan a JOIN cpcity b ON b.id=a.idCity WHERE (a.vKecamatan LIKE '%".$conn->real_escape_string($term)."%') OR (b.vCity LIKE '%".$conn->real_escape_string($term)."%')";
*/

/*
for($i = 0; $i < $p; $i++) {
	if ($i==0)
		$sql .= "WHERE (a.vKecamatan LIKE '%".$conn->real_escape_string($parts[$i])."%') OR (b.vCity LIKE '%".$conn->real_escape_string($parts[$i])."%')";
	else
		$sql .= " OR (a.vKecamatan LIKE '%".$conn->real_escape_string($parts[$i])."%') OR (b.vCity LIKE '%".$conn->real_escape_string($parts[$i])."%')";
}
*/
